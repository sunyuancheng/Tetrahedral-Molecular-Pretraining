...
from ..data.collator import lego_collator
...
@dataclass
class LEGOPretrainConfig(FairseqDataclass):
    dataset_name: str = field(
        default="PCQM4M-LSC-DFT",
    ...
    lego_noise_scale: float = field(
        default=0.05,
        metadata={
            "help": "noise add on atoms for LEGO denoise pretraining"
        }
    )

    LS_perturb_ratio: float = field(
        default=0.2,
        metadata={
            "help": "the ratio for how many local structures will be perturbed"
        }
    )

    LS_perturb_strategy: str = field(
        default="none",
    ...
    lego_loss_fn: str = field(
        default="cos",
        metadata={"help": "loss function when reconstructing local structures"},
    )


@register_task("lego_pretrain", dataclass=LEGOPretrainConfig)
class LEGOPretrainTask(FairseqTask):
    """
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        batched_data = BatchedDataDataset(batched_data,
            ...
            collator_args = {
                "noise_scale": self.cfg.lego_noise_scale,
                "perturb_ratio": self.cfg.LS_perturb_ratio,
                "perturb_strategy": self.cfg.LS_perturb_strategy,
            }
        )
